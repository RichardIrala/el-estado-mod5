...
var _addPendientes = require("./componentes/addPendientes");
// import { nameComponent } from "./name-component";
var _hojaPendientes = require("./componentes/hojaPendientes");
function main() {
    _state.state.subscribe(function() {
        localStorage.setItem("state-cache", JSON.stringify(_state.state.getState()));
    });
// console.log(localStorage.getItem("state-cache"), "tamos uwu");
}
main();

},{"./state":"1Yeju","./componentes/hojaPendientes":"8OKsY","./componentes/addPendientes":"b1Rdd"}],"1Yeju":[function(require,module,exports) {
...
const state = {
    ...
    init () {
        ...
        if (!localData) // Si no hay data, que no haga nada
        return;
    ...
    setState (newState) {
        this.data = newState;
        for (const cb of this.listeners)// console.log("SOy yo", cb);
        cb();
    },
    subscribe (callback) {
...
},{}],"8OKsY":[function(require,module,exports) {
var _state = require("../state");
class addHojaPendiente extends HTMLElement {
    ...
    connectedCallback() {
        _state.state.subscribe(()=>{
            console.log(_state.state.getState(), "aaa");
            // this.reset();
            this.render();
        });
    }
    ...
    render() {
        console.log(_state.state.getState().list, "leame ahora mismo holiis");
...
console.log(_state.state.getState().list, "leame ahora mismo xd");

},{"../state":"1Yeju"}],"b1Rdd":[function(require,module,exports) {
var _state = require("../state");
class addPendiente extends HTMLElement {
    ...
    eventListeners() {
        ...
        formulario.addEventListener("submit", (e)=>{
            e.preventDefault();
            const infForm = e.target;
            const valorForm = infForm.inputAgregar.value;
            console.log(valorForm, "soy el queridisimo valor");
            if (valorForm != "") {
                _state.state.addItem(valorForm);
                e.target["inputAgregar"].value = "";
            }
        });
        quitar.addEventListener("click", (e)=>{
            e.preventDefault();
            _state.state.getState().list.pop(_state.state.getState().list.length - 1);
            _state.state.setState("remover");
        });
    }
    render() {
        var style = document.createElement("style");
...
},{"../state":"1Yeju"}]},["8wcER","h7u1C"], "h7u1C", "parcelRequiree181")
